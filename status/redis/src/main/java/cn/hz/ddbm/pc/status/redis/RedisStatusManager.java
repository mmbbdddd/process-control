...
public class RedisStatusManager implements StatusManager {
    @Autowired
    RedisTemplate<String, Node> redisTemplate;
    ...
    @Override
    public void setStatus(String flow, Serializable flowId, Node<?> flowStatus, Integer timeout, FlowContext<?, ?> ctx) throws IOException {
        redisTemplate.opsForValue().set(String.format(keyTemplate, flow, flowId), flowStatus, timeout);
    }


    @Override
    public Node getStatus(String flow, Serializable flowId) throws IOException {
