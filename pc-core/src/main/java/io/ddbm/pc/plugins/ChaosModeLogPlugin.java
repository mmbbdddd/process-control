...
public class ChaosModeLogPlugin implements Plugin {
    Logger logger = LoggerFactory.getLogger("chaos");
    ...
    @Override
    public void onPluginFlowException(Throwable e, FlowContext ctx) {
        String flowName = ctx.getFlow().getName();
        FlowRequest<?> request = ctx.getRequest();
        logger.error("{},{}:{}", flowName, request.getDateId(), e.getMessage());
    }
