...
public class ChaosMode extends RunMode {
    public ChaosMode(String desc) {
        super(desc);
    }
    ...
    public boolean isRun(FlowContext ctx) {
        //          流程状态为结束，暂停 ，取消
        if (Objects.equals(FlowStatus.DONE, ctx.getStatus().getKey()) || Objects.equals(FlowStatus.CANCELED,
            ctx.getStatus().getKey())) {
            return false;
        }
        //节点不存在，或者节点为结束节点
        try {
